...
class AlleleSelector:
    ...
    def parse_candidate_sites(self, candidate_sites):
        ...
        for r,record in enumerate(candidate_sites):
            ...
            alleles = self._select_alleles(record["candidate_alleles"])
    ...
    def test_allele_distributions(self, vcf_handler, candidate_sites):
        ...
        :param vcf_handler: vcf querying module (assumed 1-based)
        :param candidate_sites: list of candidates
        :return:
        '''
        # find start and stop position of region that covers all candidates
        start_record = candidate_sites[0]
        ...
        self._bin_distributions_by_variant_status(variants=variants, candidate_sites=candidate_sites, vector_cutoff=8)
    ...
    def get_labeled_candidate_sites(self, vcf_handler, candidate_sites, filter_hom_ref=False):
        ...
        labeled_candidates = self._get_labeled_candidates(variants=variants, candidate_sites=candidate_sites)
    ...
    def _get_labeled_candidates(self, variants, candidate_sites):
        '''
    ...
    def _print_site_data(self, candidate_site, alt1_sequence, alt2_sequence):
    ...
    def _bin_distributions_by_variant_status(self, variants, candidate_sites, vector_cutoff):
        '''
        Finds the candidate allele window that contains each variant
        :param variants: dictionary of start:VariantRecord
        ...
        while v < len(variant_keys) and a < len(candidate_sites):
            ...
            if frequency_vector is None:
                ...
                frequency_vector = self._get_allele_frequency_vector(allele_list=candidate_sites[a]["candidate_alleles"],
                                                                     ref_sequence=ref_sequence)
